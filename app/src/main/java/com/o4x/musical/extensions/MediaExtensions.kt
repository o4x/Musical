...
import com.o4x.musical.playback.players.MediaSessionCallback
...
fun getExtraBundle(queue: LongArray, title: String, seekTo: Int?): Bundle? {
    val bundle = Bundle()
    bundle.putLongArray(MediaSessionCallback.SONGS_LIST, queue)
    bundle.putString(MediaSessionCallback.QUEUE_TITLE, title)
    if (seekTo != null)
        bundle.putInt(MediaSessionCallback.SEEK_TO_POS, seekTo)
    else bundle.putInt(MediaSessionCallback.SEEK_TO_POS, 0)
    return bundle
}
